...
import { ftGetTokenMetadata, TokenMetadata, ftGetStorageBalance } from '@services/ft-contract';
...
export const getLPTokenId = (farm_id: string) => farm_id.slice(farm_id.indexOf('@') + 1, farm_id.lastIndexOf('#'));
...
export const getFarms = async ({
	...
	const pool_ids = farms.map(f => getLPTokenId(f.farm_id));
...
export const getFarmInfo = async (
	...
	const isSignedIn: boolean = getWallet().isSignedIn();
	...
	const userRewardNumberPerWeek =
		seedAmount !== '0' ? math.round(math.evaluate(`${rewardNumberPerWeek} * (${staked ?? 0} / ${seedAmount})`)) : 0;
...
export const getUnclaimedFarms = async ({
	...
	const isSignedIn = getWallet().isSignedIn();
	...
	await Promise.all(
		farms.map(async (farm: any, i: number) => {
			const current_user_reward = isSignedIn ? await getUnclaimedReward(farm.farm_id) : 0;
...
export const claimRewardBySeed = async (seed_id: string, useFluxusFarmContract = false): Promise<any> =>
	refFarmFunctionCall({
...
export const getAllSinglePriceByTokenIds = async (token_ids: string): Promise<any> => currentTokensPrice(token_ids);

export const claimAndWithDrawReward = async (farmsData: any[]): Promise<any> => {
