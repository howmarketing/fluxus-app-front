...
class RefDatabase extends Dexie {
	...
	public constructor() {
		...
		this.version(5.1).stores({
			...
			pools_tokens: 'id, token1Id, token2Id, token1Supply, token2Supply, fee, shares, update_time, token0_price',
	...
	public searchPools(args: ISearchPoolsArgs, pools: Pool[]): Pool[] {
		if (args.tokenName === '') return pools;
		const response = _.filter(
			pools,
			(pool: Pool) => _.includes(pool.token1Id, args?.tokenName) || _.includes(pool.token2Id, args?.tokenName),
		);
		return response;
	}
	...
	public async queryPools(args: any) {
		const pools = await this.allPools().toArray();
	...
	public searchTokens(args: any, tokens: TokenMetadata[]): TokenMetadata[] {
		...
		return _.filter(tokens, (token: TokenMetadata) => _.includes(token.name, args.tokenName));
	}
